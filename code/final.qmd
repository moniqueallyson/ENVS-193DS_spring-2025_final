...
model_prediction <- ggpredict(mod2, 
...
**Figure 1. Swift Parrots have higher likelihood for box occupancy when close to forest edge:** Extracted data from the DRYAD digital repository (Saunders DL, Mawson PR, Dawson R,2020). Lines show predictions from a saturated generalized linear model with shaded ribbons indicating 95% confidence intervals, differentiated by season through the use of violet for 2016 and purple for 2019. The circular points represent the likelihood of a Swift Parrot occupancy in a nest box according to the distance from the forest edge, while triangular points represent the binary data representing occupancy.
